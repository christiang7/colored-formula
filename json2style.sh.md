...
*make.sh*
```bash
noweb.py -Rjson2style.sh json2style.sh.md > json2style.sh && echo 'json2style.sh' && notify-send -a "Compilation of json2style.sh" "" "$(date +"%Y-%m-%d") fertig" 
```


```bash
chmod u+x json2style.sh && ln -sf $(pwd)/json2style.sh ~/.local/bin/json2style.sh && echo 'fertig'
```
